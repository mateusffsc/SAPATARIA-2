/*
  ...
  This migration adds a reference_number column to the financial_transactions table
  to store the OS number, sale number, bill number, etc. This allows for easier
  identification and linking of transactions to their source records.
*/

-- Add reference_number column to financial_transactions if it doesn't exist
...
BEGIN
  ...
  FOR order_rec IN 
    ...
    WHERE ft.reference_type = 'order' AND (ft.reference_number IS NULL OR ft.reference_number = '')
  LOOP
  ...
  FOR sale_rec IN 
    ...
    WHERE ft.reference_type = 'sale' AND (ft.reference_number IS NULL OR ft.reference_number = '')
  LOOP
    UPDATE financial_transactions
  ...
  FOR bill_rec IN 
    ...
    WHERE reference_type = 'bill' AND (reference_number IS NULL OR reference_number = '')
  LOOP
    UPDATE financial_transactions
    SET reference_number = 'BILL-' || bill_rec.reference_id
