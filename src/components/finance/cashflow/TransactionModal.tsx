...
const TransactionModal: React.FC = () => {
  ...
  // Find the Caixa Loja account on load
  useEffect(() => {
  ...
  const handleSaveTransaction = async () => {
    try {
      ...
      // Find the Caixa Loja account
      const caixaLojaAccount = bankAccounts.find(acc => acc.name === 'Caixa Loja');
      if (!caixaLojaAccount) {
        alert('Conta "Caixa Loja" não encontrada. Por favor, configure as contas bancárias.');
        return;
      }
      ...
      if (transactionForm.type === 'transfer') {
        ...
        // For regular transactions, always use Caixa Loja account
        if (transactionForm.type === 'income') {
          transactionData.destination_account_id = caixaLojaAccount.id;
        } else {
          transactionData.source_account_id = caixaLojaAccount.id;
        }
      }
        ...
        <div className="p-6 space-y-4">
          ...
          {transactionForm.type === 'transfer' ? (
            ...
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
              <p className="text-sm text-blue-700 font-medium">Conta: Caixa Loja</p>
              <p className="text-xs text-blue-600 mt-1">
                Todas as transações são automaticamente registradas na conta Caixa Loja
              </p>
            </div>
          )}
