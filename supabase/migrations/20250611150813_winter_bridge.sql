/*
  # Bank Accounts and Transfers System

  1. New Table
    - `bank_accounts` - For tracking different bank accounts and their balances
  
  2. Updates to financial_transactions
    - Add columns for source and destination accounts
    - Update type check constraint to include 'transfer'
    - Add function to handle transfers between accounts
    - Add trigger to update account balances automatically
...
-- Create policy for public access (checking if it exists first)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'bank_accounts' AND policyname = 'Allow all operations on bank_accounts'
  ) THEN
...
-- Add columns to financial_transactions table for account transfers
DO $$
BEGIN
  IF NOT EXISTS (
